...
class SettingsVM extends ChangeNotifier {
  ...
  Future<void> changePassword(String oldPass, String newPass) async {
    ...
    if (user == null) {
      ...
      AuthCredential cred =
          EmailAuthProvider.credential(email: FirebaseAuth.instance.currentUser?.email ?? "", password: oldPass);
      UserCredential uCred = await user.reauthenticateWithCredential(cred).catchError((error) {
        ...
        if (er.contains("wrong-password")) {
          ZBotToast.showToastError(message: LocalizationMap.getTranslatedValues("please_provide_valid_password"));
        }
      });
      if (uCred.user?.email != null) {
        try {
          await user.updatePassword(newPass);
          ZBotToast.showToastSuccess(message: LocalizationMap.getTranslatedValues("password_changed_successfully"));
        } catch (e) {
  ...
  Future<void> updateContent(String id, String text,) async {
    try {
  ...
  Future<void> updateServiceFee(double referralAmount,double serviceFee,double tax,double tip,) async {
    try {
      ZBotToast.loadingShow();
