...
late BookinsgDataGridSource bookingGridSource;

class BookingGrid extends SampleView {
  const BookingGrid({Key? key}) : super(key: key);
...
class UserGridState extends SampleViewState {
  ...
  void initState() {
    bookingGridSource =
        BookinsgDataGridSource(isWebOrDesktop: isWebOrDesktop);
    super.initState();
  ...
  Widget _buildDataPager() {
    return SfDataPagerTheme(
      ...
      child: SfDataPager(
        ...
        delegate: bookingGridSource,
        availableRowsPerPage: const <int>[10, 20, 25],
        visibleItemsCount: 5,
        pageCount:
            (bookingGridSource.tickets.length / _rowsPerPage).ceilToDouble(),
