...
import 'package:yacht_master_admin/src/dashboard/pages/invites/view/widgets/invites_grid.dart';
...
class InvitesDataGridSource extends DataGridSource {
  /// Creates the order data source class with required details.
  InvitesDataGridSource({this.model, required this.isWebOrDesktop}) {
    ...
    if (vm.selectedIndex == 3) {
      ...
      dataGridRows = tickets
          .where((element) => element.firstName
              ...
              .isCaseInsensitiveContains(vm.searchController.text))
          .where((element) =>
              element.inviteStatus == vm.selectedIndex &&
              (element.inviteStatus!) > 0)
          .toList()
          .map<DataGridRow>((UserModel model) {
        return DataGridRow(cells: <DataGridCell>[
        ...
        PopupMenuButton(
          ...
          onSelected: (val) async {
            ...
            requestDataGridSource =
                InvitesDataGridSource(isWebOrDesktop: isWebOrDesktop);
            Get.forceAppUpdate();
          },
          itemBuilder: (context) => model.inviteStatus == 1
              ? <PopupMenuItem>[
