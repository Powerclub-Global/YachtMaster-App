...
class _AdminChatViewState extends State<AdminChatView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer2<BaseVm, InboxVm>(
      builder: (context, baseVm, model, _) {
              ...
              child: Directionality(
                ...
                child: AppBar(
                  ...
                  title: GestureDetector(
                    onTap: () {
                      // Get.toNamed(HostProfileOthers.route,arguments: {"host":baseVm.allUsers.firstWhereOrNull((element) => chatHeadModel?.users?.where((element) => element!=firebaseAuthService.currentUserId).first==element.uid)});
                    },
                    child: Text(
                      baseVm.allUsers
                              .firstWhereOrNull(
  ...
  Widget conversation(InboxVm model, BaseVm baseVm) {
    return Expanded(
      child:
          baseVm.allUsers.isEmpty
              ? SizedBox()
              : StreamBuilder(
                stream:
                    FbCollections.message(
                      chatHeadModel?.id,
              ...
              child: Stack(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      DateFormat.jm()
                          .format(chatModel.createdAt?.toDate() ?? now)
                          .toString()
                          .toLowerCase(),
                      style: R.textStyle.helvetica().copyWith(
                        color:
  ...
  Widget receiverBubble(AdminChatModel chatModel, BaseVm baseVm) {
    return baseVm.allUsers.firstWhereOrNull(
              (element) =>
                  element.uid ==
              ...
              onTap: () async {
                ...
                if (msgCon.text.trim().isEmpty) {
                  ...
                  try {
                    ...
                    msgCon.clear();
                    scrollController?.animateTo(
                      (scrollController?.position.maxScrollExtent ?? 0) * 1000,
