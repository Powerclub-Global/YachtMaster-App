...
class _BecomeHostState extends State<BecomeHost> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: GeneralAppBar.simpleAppBar(context, getTranslated(context, "become_a_host")??""),
             ...
             "In order to receive payouts greater than \$400 per calendar year Yacht Master App must collect a 1099 Tax form as mandated by the federal Tax Commission.\n\nPlease complete and Submit your 1099 Tax from to request host access.",
              style: R.textStyle.helvetica().copyWith(
                fontWeight: FontWeight.w600,
                  height: 1.5, color: Colors.white, fontSize: 12.sp),
              textAlign: TextAlign.center,
            ),
          ),
          Spacer(),
         if(screenShot!=null) DottedBorder(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Flexible(
                      child: Row(
                        children: [
                          ...
                          w3,
                          Flexible(
                            child: Text(screenShot?.fileName??"",style: R.textStyle.helvetica().copyWith(
                    ...
                    GestureDetector(
                      ...
                      child: Icon(Icons.cancel_outlined,color: R.colors.whiteColor,
                        size: 24,),
                    )
                  ],
                ),
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () {
                      ...
                      _flutterMediaDownloaderPlugin.downloadMedia(context,context.read<BookingsVm>().appUrlModel?.hostPolicies??"",).then((value) {
                        ZBotToast.showToastSuccess(message: "Host Policy downloaded successfully!",);
                      });
          ...
          GestureDetector(
            onTap: () async {
              ...
              if(screenShot==null || screenShot?.fileName=="")
                {
                  ZBotToast.showToastError(message: "Please upload the required document mentioned in the Host Policy");
                }
              else{
                ...
                ZBotToast.showToastSuccess(message: "Request has been sent to admin.Please wait for the approval!");
                Get.back();
