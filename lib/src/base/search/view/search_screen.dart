...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      yachtVm.allCharters.where((element) => homeVm.allBookings.where((bookin) => bookin.charterFleetDetail?.id==element.id).toList().length==10).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer5<HomeVm,SettingsVm,YachtVm, SearchVm, BookingsVm>(
        builder: (context, homeVm,settingsVm,yachtVm, provider, bookingVm, _) {
                              ...
                              children: [
                                ShaderMask(
                                  ...
                                  child: ClipRRect(
                                      ...
                                      child: Image.asset(R.images.searchBack)),
                                ),
                                Text(
                                ...
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: TextFormField(
                                      ...
                                      style: R.textStyle.helvetica().copyWith(
                                          ...
                                          fontSize: 11.sp),
                                      decoration: AppDecorations.greyTextField("city",
                                  ...
                                  Center(
                                    child: GestureDetector(
                                      onTap: () async {
                                        if (provider.selectedCharterDayType == null) {
                                          Helper.inSnackBar(
                                              "Error",
                                              "Please select charter type",
                                              R.colors.themeMud);
                                        }
                                        else if (bookingVm.totalMembersCount == 0) {
                                          Helper.inSnackBar(
                                              "Error",
                                          ...
                                          var bookingsVm = Provider.of<BookingsVm>(
                                              context,
                                              listen: false);
                                          bookingsVm.bookingsModel = BookingsModel(schedule: BookingScheduleModel(dates: []));
                                          provider.selectedCity=cityCon.text;
                                          log("________________START DATE:${provider.startDate}____END:${provider.endDate}");
                                          List<DateTime> days =provider.endDate==null?[provider.startDate??now]: List.from(
                                              provider.startDate?.getDays(provider.startDate??now,provider.endDate!)??[]);
                                          log("________________DAYS:${days}");
                                          provider.selectedBookingDays = days.toSet();
                                          log("________________SELECTE DAT:${provider.selectedBookingDays}");
                                          bookingsVm.bookingsModel.totalGuest = bookingVm.totalMembersCount;
                                          bookingsVm.selectedPaymentMethod = -1;
                                          bookingsVm.bookingsModel.durationType =
                                              provider.selectedCharterDayType?.type;

                                          bookingsVm.bookingsModel.schedule?.dates=List.from(provider.selectedBookingDays?.toList().map((e) => Timestamp.fromDate(e)).toList()??[]);
                                          bookingsVm.update();
                                          bookingsVm.bookingsModel.schedule?.startTime =
                                              startTimeCon.text;
                                          if(provider.selectedCharterDayType?.type==CharterDayType.halfDay.index)
                                          {
                                            DateTime endTime=bookingVm.startTime!.add(Duration(hours: 4));
                                            endTimeCon.text= DateFormat('hh:mm a').format(endTime);
                                            bookingsVm.bookingsModel.schedule?.endTime =endTimeCon.text;
                                          }else if(provider.selectedCharterDayType?.type==CharterDayType.fullDay.index)
                                          {
                                            DateTime endTime=bookingVm.startTime!.add(Duration(hours: 8));
                                            endTimeCon.text= DateFormat('hh:mm a').format(endTime);
                                            bookingsVm.bookingsModel.schedule?.endTime =endTimeCon.text;
                                          }
                        ...
                        children: [
                            ...
                            GeneralWidgets.seeAllWidget(context, "charters",
                                onTap: () {
                                  Get.toNamed(SearchSeeAll.route,
                                      arguments: {"isReserve": false, "index": 0,"seeAllType":SeeAllType.charter.index});
                                }),
                            h2,
                            Padding(
                              ...
                              child: SingleChildScrollView(
                                ...
                                child: Row(
                                  children: List.generate(
                                    ...
                                    3: yachtVm.allCharters.length, (index) {
                                    // CharterModel charterModel = yachtVm.allCharters.where((element) => homeVm.allBookings.where((bookin) => bookin.charterFleetDetail?.id==element.id).toList().length>=(bookingVm.appUrlModel?.superhostminimumbookings??0)).toList()[index];
                                     CharterModel charterModel = yachtVm.allCharters[index];
                                    return Padding(
                                      padding: EdgeInsets.only(right: 10),
                                      child: GestureDetector(
                                          onTap: () {
                                            bookingVm.bookingsModel=BookingsModel();
                                            bookingVm.bookingsModel.durationType = CharterDayType.halfDay.index;
                                            ...
                                            Get.toNamed(CharterDetail.route,
                                                arguments: {
                                                  ...
                                                  "isReserve": false,
                                                  "index": index,
                                                  "isEdit":charterModel.createdBy==FirebaseAuth.instance.currentUser?.uid?true: false
                                                });
                                          },
                                          child: CharterWidget(
                                            ...
                                            isSmall: true,
                                            isShowStar: true,
                                            isFav: yachtVm.userFavouritesList.any((element) => element.favouriteItemId==charterModel.id && element.type==FavouriteType.charter.index),
                                            isFavCallBack: () async {
                                              ...
                                              if(yachtVm.userFavouritesList.any((element) => element.id==charterModel.id))
                                              {
                                                yachtVm.userFavouritesList.removeAt(index);
                                                yachtVm.update();
                                                await FbCollections.user.doc(FirebaseAuth.instance.currentUser?.uid).collection("favourite").doc(charterModel.id).delete();
                                              }
                                              else
                                              {
                                                await FbCollections.user.doc(FirebaseAuth.instance.currentUser?.uid).collection("favourite").doc(charterModel.id).set(favModel.toJson());
                                              }
                                              provider.update();
                                            },
                                          )),
                          ...
                          if (yachtVm.allServicesList.isEmpty)
                            ...
                            GeneralWidgets.seeAllWidget(context, "experiences",
                                onTap: () {
                                  Get.toNamed(SearchSeeAll.route,
                                      arguments: {"isReserve": false, "index": 1,"seeAllType":SeeAllType.service.index});
                                }),
                                        ...
                                        return GestureDetector(
                                          onTap: () {
                                            Get.toNamed(ServiceDetail.route,
                                                arguments: {
                                                  "service": service,
                                                  "isHostView":service.createdBy==FirebaseAuth.instance.currentUser?.uid?true:  false,
                                                  "index":index
                                                });
                                          },
                                          child: Padding(
                                            ...
                                            child: HostWidget(
                                              ...
                                              isShowRating: false,
                                              isShowStar: true,
                                              isFav: yachtVm.userFavouritesList.any((element) => element.favouriteItemId==service.id && element.type==FavouriteType.service.index),
                                              isFavCallBack: () async {
                                                FavouriteModel favModel=FavouriteModel(
                                                    creaatedAt: Timestamp.now(),
                                                ...
                                                if(yachtVm.userFavouritesList.any((element) => element.id==service.id))
                                                {
                                                  yachtVm.userFavouritesList.removeAt(index);
                                                  yachtVm.update();
                                                  await FbCollections.user.doc(FirebaseAuth.instance.currentUser?.uid).collection("favourite").doc(service.id).delete();
                                                }
                                                else
                                                {
                                                  await FbCollections.user.doc(FirebaseAuth.instance.currentUser?.uid).collection("favourite").doc(service.id).set(favModel.toJson());
                          ...
                          if (yachtVm.allYachts.isEmpty)
                            ...
                            GeneralWidgets.seeAllWidget(context, "yacht_for_sale",
                                onTap: () {
                                  Get.toNamed(SearchSeeAll.route,
                                      arguments: {"isReserve": false, "index": 3,"seeAllType":SeeAllType.yacht.index});
                                }),
                          h2,
                          if (yachtVm.allYachts.isEmpty)
                            SizedBox()
                                  ...
                                  return GestureDetector(
                                    onTap: () {
                                      Get.toNamed(YachtDetail.route, arguments: {
                                        ...
                                        "isEdit":yachtVm.allYachts[index].createdBy==FirebaseAuth.instance.currentUser?.uid?true: false,
                                        "index": -1
                                      });
                                    },
                                    child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Center(
                              child: GestureDetector(
                                onTap: () async {
                                  AuthVm authVm=Provider.of(context,listen: false);
                                ...
                                child: Container(
                                  ...
                                  child: Center(
                                    child: Text(
                                      getTranslated(context, context.read<AuthVm>().userModel?.requestStatus==RequestStatus.host ? "host_profile" : "become_a_host").toString(),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                h1P5,
                                supportWidget("help_center", "get_support", 5,settingsVm),
                                supportWidget("health_and_safety","covid_responses", 7,settingsVm),
                              ],
                            ),
                          ),
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                supportWidget("refund_policy", "how_you_are_protected", 8,settingsVm),
                                supportWidget("cancellation_options", "learn_our_policy", 9,settingsVm),
  ...
  Widget supportWidget(String title,String subtitle,int index,SettingsVm settingsVm)
  {
        ...
        GestureDetector(
          onTap: () {
            ...
            Get.toNamed(RulesRegulations.route, arguments: {
              "appBarTitle":
              settingsVm.allContent.where((element) => element.type==index).first.title??"",
              "title": "",
              "desc":settingsVm.allContent.where((element) => element.type==index).first.content??"",
    ...
    return GestureDetector(
      ...
      child: Stack(
        children: [
          ...
          if (user.uid==FirebaseAuth.instance.currentUser?.uid) SizedBox() else Positioned(
              top: 1,
              right: 3.w,
              child: GestureDetector(
                onTap: isFavCallBack,
  ...
  Future<ChatHeadModel?> createChatHead(InboxVm chatVm,YachtVm yachtVm) async {
    ...
    List<String> tempSort = [FirebaseAuth.instance.currentUser?.uid ?? "", yachtVm.allHosts.first.uid??""];
    tempSort.sort();
