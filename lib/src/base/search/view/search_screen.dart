...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      List<BookingsModel> pendingReviewsList =
          pendingReviews.docs
              .map(
                (e) => BookingsModel.fromJson(e.data() as Map<String, dynamic>),
              )
              .toList()
              .where(
                (element) =>
      ...
      if (pendingReviewsList.isNotEmpty) {
        ...
        Get.bottomSheet(
          FeedbackSheet(
            ...
            submitCallBack: (rat, desc, tipAmount) async {
              log(
                "____________RATING:${rat}____:${desc}____${bookingsModel.id}",
              );
              firstSlpliter?.paymentStatus = PaymentStatus.ratingDone.index;
              if (bookingsModel.paymentDetail?.splitPayment?.every(
                    (element) =>
                        element.paymentStatus == PaymentStatus.ratingDone.index,
                  ) ==
                  true) {
                          ...
                          Stack(
                            ...
                            children: [
                              ...
                              SizedBox(
                                width: Get.width,
                                child: Divider(
                                  color: R.colors.grey.withValues(alpha: .70),
                                  height: 0,
                                ),
                              ),
                            ],
                              ...
                              children: [
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      if (provider
                                        ...
                                        Container(
                                          width: 2,
                                          color: R.colors.grey.withValues(
                                            alpha: .40,
                                          ),
                                          height: 20,
                                        ),
                                ...
                                DecoratedBox(
                                  ...
                                  child: TextFormField(
                                    ...
                                    validator:
                                        (val) => FieldValidator.validateEmpty(
                                          val ?? "",
                                        ),
                                    decoration: AppDecorations.greyTextField(
                                      "start_time",
                                  ...
                                  child: GestureDetector(
                                    onTap: () async {
                                      if (provider.selectedCharterDayType ==
                                        ...
                                        var bookingsVm =
                                            Provider.of<BookingsVm>(
                                              context,
                                              listen: false,
                                            );
                                        bookingsVm
                                            .bookingsModel = BookingsModel(
                                          schedule: BookingScheduleModel(
                                            dates: [],
                                          ),
                                        );
                                        provider.selectedCity = cityCon.text;
                                        log(
                                          "________________START DATE:${provider.startDate}____END:${provider.endDate}",
                                        );
                                        List<DateTime> days =
                                            provider.endDate == null
                                                ? [provider.startDate ?? now]
                                                : List.from(
                                                  provider.startDate?.getDays(
                                                        provider.startDate ??
                                                            now,
                                                        provider.endDate!,
                                                      ) ??
                                        ...
                                        provider.selectedBookingDays =
                                            days.toSet();
                                        log(
                                          "________________SELECTE DAT:${provider.selectedBookingDays}",
                                        );
  ...
  Widget host(UserModel user, int index, bool isFav, Function() isFavCallBack) {
    return GestureDetector(
      onTap: () {
        user.uid == firebaseAuthService.currentUserId
            ? Get.toNamed(HostProfile.route)
      ...
      child: Stack(
        children: [
              ...
              children: [
                Container(
                  ...
                  child: ClipRRect(
                    ...
                    child:
                        user.imageUrl == ""
                            ? Image.network(R.images.dummyDp, fit: BoxFit.cover)
                            : Image.network(
          ...
          if (user.uid == firebaseAuthService.currentUserId)
            SizedBox()
          else
            Positioned(
              top: 1,
