...
class _PayWithWalletState extends State<PayWithWallet> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      {
        splitPerson=bookingsVm.bookingsModel.paymentDetail?.splitPayment?.where((element) => element.userUid==FirebaseAuth.instance.currentUser?.uid).first;
      }
    ...
    return Consumer<BookingsVm>(
        builder: (context, provider, _) {
          ...
          return Scaffold(
            ...
            appBar: GeneralAppBar.simpleAppBar(context, "${getTranslated(context, "pay_with_wallet").toString().capitalize}"),
            body: Padding(
              ...
              child: Form(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    h2,
                    Text(getTranslated(context, "amount_you_want_to_pay")??"",style: R.textStyle.helvetica().copyWith(
                        color: Colors.white
                    ),),
                    h3,
                    TextFormField(
                  ...
                  if(walletAmount>=double.parse(amountCon.text))
                    {
                      ...
                      provider.bookingsModel.paymentDetail?.payWithWallet=provider.bookingsModel.paymentDetail?.payWithWallet+double.parse(amountCon.text);
                      provider.bookingsModel.paymentDetail?.paidAmount=provider.bookingsModel.paymentDetail?.paidAmount+double.parse(amountCon.text);
                      provider.bookingsModel.paymentDetail?.remainingAmount=
                      provider.bookingsModel.paymentDetail?.payInType==PayType.deposit.index && provider.bookingsModel.paymentDetail?.isSplit==true?
                          percentOfAmount(provider.bookingsModel.paymentDetail?.remainingAmount,25)-double.parse(amountCon.text):
                          provider.bookingsModel.paymentDetail?.remainingAmount-double.parse(amountCon.text);
                      if(  provider.bookingsModel.paymentDetail?.isSplit==true)
                        {
                          splitPerson?.payWithWallet=splitPerson?.payWithWallet+double.parse(amountCon.text);
                          ...
                          provider.bookingsModel.paymentDetail?.payInType==PayType.deposit.index && provider.bookingsModel.paymentDetail?.isSplit==false?
                              percentOfAmount(splitPerson?.remainingAmount,25)-double.parse(amountCon.text):
                              splitPerson?.remainingAmount-double.parse(amountCon.text);
                          splitPerson?.amount=
                      ...
                      var authVm=Provider.of<AuthVm>(context,listen: false);
                      authVm.wallet?.amount=authVm.wallet?.amount-double.parse(amountCon.text);
                      Get.forceAppUpdate();
                      Get.back();
                    }
                  else{
                    Helper.inSnackBar("Error", "You do not have such amount in your wallet", R.colors.themeMud);
                  }
