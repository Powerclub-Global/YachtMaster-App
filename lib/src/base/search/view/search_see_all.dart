...
class _SearchSeeAllState extends State<SearchSeeAll> {
  ...
  placeFilter(int placeIndex) async {
    ...
    setState(() {});
    log(
      "______________Selected index:${placeIndex}_____${placeFilterList[placeIndex]}",
    );
    await yachtVm.allCharters.asyncForEach((element) async {
  ...
  searchPlace(List<dynamic> list) async {
    ...
    log(
      "_______________AFTER NEW LAT LNG:${provider.newLocationLatLng}___${provider.city}",
    );
    list.forEach((element) async {
      if (seeAllType == SeeAllType.service.index) {
        log(
          "______________lat:${element.location?.lat}____${element.location?.log}_____${element.name}",
        );
      ...
      if (seeAllType == SeeAllType.charter.index || seeAllType == -1) {
        await yachtVm.allCharters.asyncForEach((charterElement) async {
          ...
          if (seeAllType == -1) {
            ...
            log(
              "_______________${charterElement.location.city.toString().toLowerCase()}____${pro.selectedCity?.toLowerCase().removeAllWhitespace}___",
            );
            if (charterElement.location.city
              ...
              log(
                "__________________SEE ALL TYPE:${provider.bookingsModel.schedule?.startTime}___${provider.bookingsModel.schedule?.endTime}______${seeAllType}___${charterElement.location.city}___${pro.selectedCity}____${charterElement.name}",
              );
              List<HalfDaySlots> halfDaySlots = [];
              List<FullDaySlots> fullDaySlots = [];
              charterElement?.availability?.halfDaySlots?.forEach((
                halfSlotEle,
              ) {
                String charterStart =
              ...
              charterElement?.availability?.fullDaySlots?.forEach((
                fullDaySlot,
              ) {
                String charterStart =
              ...
              log(
                "___________NEW HALF DAYS:${halfDaySlots.map((e) => e.start)}____${fullDaySlots.length}",
              );
              if ((provider.bookingsModel.durationType ==
                  ...
                  (provider.bookingsModel.durationType ==
                ...
                List<DateTime> bookedDates = await getBookedDates(
                  charterElement,
                );
                ...
                if (bookedDates.isNotEmpty) {
                  ...
                  charterElement.availability.dates.forEach((date) {
                    log(
                      "________BOOKED:${bookedDates.first}____CHARTE DATE:${date.toDate()}",
                    );
                  ...
                  availableDates = List.from(
                    charterElement.availability.dates
                            .map((e) => e.toDate())
                            .toList() ??
                        [],
                  );
                ...
                log(
                  "___________CHARTER CITIES:${charterElement.location.city}____Charter nane:${charterElement.name}",
                );
                if (provider.bookingsModel.schedule?.dates?.every(
                      (element) => availableDates.contains(element.toDate()),
                    ) ==
                    true) {
                  log(
                    "___________TRUE${charterElement.location?.lat}____${charterElement.location?.long}",
                  );
                  await moveToLocation(
                    LatLng(
                      charterElement.location?.lat ?? 0.0,
        ...
        await yachtVm.allServicesList.asyncForEach((serviceElement) async {
          mostOccurredCity.clear();
          String moveToCity = await getMostOccurredCity(
          ...
          await moveToLocation(
            LatLng(
              serviceElement.location?.lat ?? 0.0,
  ...
  Future<List<DateTime>> getBookedDates(CharterModel? charter) async {
    ...
    await homeVm.allBookings.asyncForEach((element) async {
      log(
        "________BOOKING iD:${element.id}_______${element.charterFleetDetail == charter?.id}______${element.bookingStatus == BookingStatus.ongoing.index}___${(element.schedule?.dates?.length ?? 0) > 1}",
      );
  ...
  Widget charterPopups(CharterModel charterModel) {
                    ...
                    child: Wrap(
                      ...
                      children: List.generate(
                        selectedPlace != null
                            ...
                            : selectedPrice != null
                            ? priceFilterListView(selectedPrice ?? 0).length
                            : yachtVm.allCharters.length,
                        (index) {
                          CharterModel model =
                              selectedPlace != null
                                  ? placeFilterListView(
                ...
                child: SingleChildScrollView(
                  child: Center(
                    child: Wrap(
                      ...
                      crossAxisAlignment: WrapCrossAlignment.start,
                      alignment: WrapAlignment.start,
                      children: List.generate(yachtVm.allYachts.length, (
                        index,
                      ) {
  ...
  void setMarker(LatLng latLng, dynamic charterModel) {
    // markers.clear();
    log(
      "__________________IN SET MARLKER ID:${charterModel.id ?? ""}____${charterModel.name}",
    );
    setState(() {
      marker.add(
        Marker(
