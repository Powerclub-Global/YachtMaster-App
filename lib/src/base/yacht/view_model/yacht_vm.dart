...
class YachtVm extends ChangeNotifier {
 ...
 CharterModel? charterModel=CharterModel(availability: Availability(dates: []));
...
List<CharterModel> hostCharters=[];
 ...
 Future<void> fetchServices() async {
  ...
  var ref =  FbCollections.services
      ...
      .snapshots()
      .asBroadcastStream();
  var res = ref.map((list) => list.docs.map((e) => ServiceModel.fromJson(e.data())).toList());
  try {
    servicesStream ??= res.listen((services) async {
     if (services.isNotEmpty) {
      allServicesList = services.where((element) =>  element.status==0).toList();
      log("__________HOST UID:${FirebaseAuth.instance.currentUser?.uid}");
      hostServicesList=services.where((element) => element.createdBy==FirebaseAuth.instance.currentUser?.uid && element.status==0).toList();
      notifyListeners();
      log(
          "//////////////////////////////////////////////All SERVICES :${allServicesList.length}/////host SERVICES :${hostServicesList.length}/////${FirebaseAuth.instance.currentUser?.uid}");
     }
     notifyListeners();
    });
 ...
 Future<void> fetchCharters() async {
  ...
  try {
    hostCharters=[];
    allCharters=[];
    var ref = FbCollections.charterFleet
        .snapshots()
        .asBroadcastStream();
    var res = ref.map((list) => list.docs.map((e) => CharterModel.fromJson(e.data())).toList());
    charterStream ??= res.listen((charters) async {
     if (charters.isNotEmpty) {
        allCharters = charters.where((element) =>  element.status==CharterStatus.active.index).toList();
        hostCharters=charters.where((element) => element.createdBy==FirebaseAuth.instance.currentUser?.uid && element.status==CharterStatus.active.index).toList();
 ...
 Future<List<UserModel>?> fetchAllHost(List<CharterModel> charters)
 async {
  ...
  sortHostsByBookings()async
 {
  ...
  allHosts.forEach((hostElement) {
   ...
   homeVm.allBookings.forEach((element) {
    ...
    if((element.hostUserUid==hostElement.uid && element.bookingStatus==BookingStatus.completed.index)==true)
    {
     count=count+1;
 ...
 Future<void> fetchYachts() async {
  try {
    ...
    hostYachts=[];
    allYachts=[];
    var ref = FbCollections.yachtForSale
        .snapshots()
        .asBroadcastStream();
    var res = ref.map((list) => list.docs.map((e) => YachtsModel.fromJson(e.data())).toList());
    yachtStream ??= res.listen((yachts) async {
     if (yachts.isNotEmpty) {
        ...
        hostYachts=yachts.where((element) => element.createdBy==FirebaseAuth.instance.currentUser?.uid && element.status==0).toList();
        notifyListeners();
 ...
 Future<void> fetchUserFavourites() async {
  ...
  userFavouritesList=[];
  var ref = FbCollections.user.doc(FirebaseAuth.instance.currentUser?.uid).collection("favourite")
      .snapshots()
      .asBroadcastStream();
  var res = ref.map((list) => list.docs.map((e) => FavouriteModel.fromJson(e.data())).toList());
  try {
  ...
  onClickAddService(String? city,bool isEdit,List<XFile> fileImages,List<String> networkImagesList,
     List<String> deletedImagesRef,
     String whatYouDoCon,String nameCon,String locationCon,LatLng? locationLatLng,BuildContext context) async {
  ...
  if(fileImages.isNotEmpty)
   {
    networkImagesList=await uploadServiceImages(fileImages,"serviceImages");
   }
  if(networkImagesList.isEmpty)
  {
   ...
   if(isEdit==true)
   {
    ...
    serviceModel?.location=LocationModel(address: locationCon,
        lat:locationLatLng?.latitude,log: locationLatLng?.longitude,city: city );
    ...
    serviceModel=
        ServiceModel(
         ...
         location:LocationModel(address: locationCon,
             lat:locationLatLng?.latitude,log: locationLatLng?.longitude,city:city ),
   ...
   Helper.inSnackBar("Success",
       isEdit==true?"Service updated successfully":  "Service added successfully", R.colors.themeMud);

  }
 }
 uploadServiceImages(List<XFile> fileImages,String bucketName)
 async {
  List<String> newUploadedImages=[];
  try {
   newUploadedImages=await ImagePickerServices().uploadPostImages(fileImages,bucketName);
   update();
  ...
  onClickAddYacht(String? city,bool isEdit,List<XFile> fileImages,List<String> networkImagesList,List<String> deletedImagesRef,
     String whatYouDoCon,String priceCon,String nameCon,String locationCon,LatLng? locationLatLng,BuildContext context) async {
  var authVm=Provider.of<AuthVm>(context,listen: false);
  var searchVm=Provider.of<SearchVm>(context,listen: false);
  ZBotToast.loadingShow();
  if(fileImages.isNotEmpty)
  ...
  if(networkImagesList.isEmpty)
  {
   ...
   if(isEdit==true)
   {
    ...
    yachtsModel?.location=YachtLocation(address: locationCon,
        lat:locationLatLng?.latitude,long: locationLatLng?.longitude ,city: city);
    ...
    yachtsModel=
        YachtsModel(
         ...
         location:YachtLocation(address: locationCon,
             lat:locationLatLng?.latitude,long: locationLatLng?.longitude,city: city ),
         description:whatYouDoCon,
        );
    try {
     await  FbCollections.yachtForSale.doc(docID).set(yachtsModel?.toJson());
    } on Exception catch (e) {
     // TODO
     ZBotToast.loadingClose();
     debugPrintStack();
   ...
   Helper.inSnackBar("Success",
       isEdit==true?"Yacht updated successfully":  "Yacht added successfully", R.colors.themeMud);

  }
 }
  onClickAddCharter(int? isPetAllow,String? city,bool isEdit,List<XFile> fileImages,List<String> networkImagesList,List<String> deletedImagesRef,
     String subheadingCon,String guestCountCon,String priceFourCon,String priceEightCon,String priceFullCon,String nameCon,String locationCon,LatLng? locationLatLng,BuildContext context) async {
  var searchVm=Provider.of<SearchVm>(context,listen: false);
  ZBotToast.loadingShow();
  if(fileImages.isNotEmpty)
   {
    networkImagesList=await uploadServiceImages(fileImages, "charterImages");
   }
  if(networkImagesList.isEmpty)
  {
   Helper.inSnackBar("Error", "Please upload images", R.colors.themeMud);
   ...
   charterModel?.subHeading =
       subheadingCon;
   charterModel?.guestCapacity =
       int.parse(guestCountCon);
   charterModel?.images=List.from(networkImagesList);
   charterModel?.location=CharterLocationModel(adress: locationCon,
       lat:locationLatLng?.latitude,long: locationLatLng?.longitude ,city: city);
   charterModel?.priceFullDay=priceFullCon.isNotEmpty ? double.parse(priceFullCon) : 0;
   charterModel?.priceHalfDay=priceEightCon.isNotEmpty ? double.parse(priceEightCon) : 0;
   charterModel?.priceFourHours=priceFourCon.isNotEmpty ? double.parse(priceFourCon) : 0;
   if(isEdit==true)
   {
    log("____________________in eidt:");
    try {
    ...
    await  FbCollections.charterFleet.doc(charterModel?.id).set(charterModel?.toJson());
    ...
    charterModel?.availability=charterModel?.availability ?? Availability();
    charterModel?.healthSafety=charterModel?.healthSafety ?? HealthSafety();
    charterModel?.cancelationPolicy=charterModel?.cancelationPolicy ?? CancelationPolicy();
    charterModel?.yachtRules=charterModel?.yachtRules ?? YachtRules();
    try {
     await  FbCollections.charterFleet.doc(docID).set(charterModel?.toJson());
     // searchVm.servicesList.add(serviceModel!);
    } on Exception catch (e) {
     // TODO
     ZBotToast.loadingClose();
