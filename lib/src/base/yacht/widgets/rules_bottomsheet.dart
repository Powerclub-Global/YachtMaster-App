...
class _RulesBottomSheetState extends State<RulesBottomSheet> {
  ...
  void initState() {
    ...
    if(widget.index==8 && (yachtVm.charterModel?.cancelationPolicy?.title!=""
        && yachtVm.charterModel?.cancelationPolicy!=null)) {
      titleCon.text=yachtVm.charterModel?.cancelationPolicy?.title??'';
      descCon.text=yachtVm.charterModel?.cancelationPolicy?.description??'';
    }
    if(widget.index==7 && (yachtVm.charterModel?.healthSafety?.title!="" && yachtVm.charterModel?.healthSafety!=null)) {
      titleCon.text=yachtVm.charterModel?.healthSafety?.title??'';
      descCon.text=yachtVm.charterModel?.healthSafety?.description??'';
    }
    if(widget.index==6 && (yachtVm.charterModel?.yachtRules?.title!="" && yachtVm.charterModel?.yachtRules!=null)) {
                    ...
                    children: <Widget>[
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            h3,
                            Text("${getTranslated(context, widget.title??"").toString().capitalize}",
                                style:R.textStyle.helveticaBold().copyWith(
                                    color:  R.colors.whiteColor,
                                    fontWeight: FontWeight.bold,
                                    fontSize: Get.width * .053)),
                            h2,
                            Text("${getTranslated(context, widget.subTitle??"").toString().capitalize}",
                                style:R.textStyle.helvetica().copyWith(
                                    color:  R.colors.whiteDull,
                                    fontWeight: FontWeight.bold,height: 1.3,
                                    fontSize: Get.width * .04),textAlign: TextAlign.center,),
                            h4,
                            Form(
                              key: formKey,
                              ...
                              child: Column(
                                children: [
                                  label(getTranslated(context,  "title",)??""),
                                  h0P5,
                                  TextFormField(
                                    focusNode: titleFn,
                                    inputFormatters: [
                            ...
                            GestureDetector(
                              onTap: () {
                                if (formKey.currentState!.validate()) {
                                  ...
                                  switch(widget.index){
                                    ...
                                    case 6:
                                      provider.charterModel?.yachtRules=YachtRules(title: titleCon.text,description: descCon.text);
                                      break;
                                    case 7:
                                      provider.charterModel?.healthSafety=HealthSafety(title: titleCon.text,description: descCon.text);
                                      break;
                                    case 8:
                                      provider.charterModel?.cancelationPolicy=CancelationPolicy(title: titleCon.text,description: descCon.text);
                                      break;
                                  }
                                  provider.update();
                                  Get.back();
                              ...
                              child:  Container(
                                height: Get.height*.06,width: Get.width*.65,
